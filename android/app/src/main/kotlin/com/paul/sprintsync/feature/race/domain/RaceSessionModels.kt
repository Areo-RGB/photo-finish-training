...
data class SessionControlCommandMessage(
    ...
    companion object {
        ...
        fun tryParse(raw: String): SessionControlCommandMessage? {
            ...
            val gameModeEnabled = if (decoded.has("gameModeEnabled") && !decoded.isNull("gameModeEnabled")) {
                decoded.optBoolean("gameModeEnabled")
            } else {
                null
            }
            ...
            if (action == SessionControlAction.SET_GAME_MODE_ENABLED && gameModeEnabled == null) {
                return null
            }
            if (
                action == SessionControlAction.SET_GAME_MODE_LIMIT &&
                (gameModeLimitMillis == null || gameModeLimitMillis <= 0L)
            ) {
                return null
            }
            if (
                action == SessionControlAction.SET_GAME_MODE_LIVES &&
                (gameModeLives == null || gameModeLives !in 1..10)
            ) {
                return null
            }
            if (
