...
class MainActivity : ComponentActivity(), ActivityCompat.OnRequestPermissionsResultCallback {
    companion object {
        ...
        private const val DEFAULT_GAME_MODE_AUTO_REDUCTION_MILLIS = 100L
        private const val MIN_GAME_MODE_LIMIT_MILLIS = 100L
        private const val TARGET_MONITORING_WIFI_SSID = "TP-Link_86CA_5G"
        private const val DISPLAY_LOCAL_SOUND_TARGET = "__display_local_sound__"
    }
    ...
    private val displayGameModeAutoReductionMillisByEndpointId = linkedMapOf<String, Long>()
    ...
    private fun onMainAction(action: MainAction) {
        when (action) {
            ...
            is MainAction.SetGameModeAutoConfig -> {
                ...
                val reductionMillis = action.reductionMillis.coerceIn(50L, 500L)
                sendControllerCommandToDisplayHost(
                    ...
                    gameModeAutoReductionMillis = reductionMillis,
                )
            }
            MainAction.PlayStartSound -> {
                sendControllerCommandToDisplayHost(
                    action = SessionControlAction.PLAY_START_SOUND,
                    targetEndpointId = DISPLAY_LOCAL_SOUND_TARGET,
                    limitMillis = null,
                    sensitivityPercent = null,
                    autoReadyDelaySeconds = null,
                    waitTextEnabled = null,
            ...
            is MainAction.UpdateRoiCenterY -> {
                motionDetectionController.updateRoiCenterY(action.value)
                syncControllerSummaries()
            }
            is MainAction.UpdateRoiHeight -> {
                motionDetectionController.updateRoiHeight(action.value)
                syncControllerSummaries()
            }
                    ...
                    is NearbyEvent.PayloadReceived -> {
                        SessionControlCommandMessage.tryParse(event.message)?.let { command ->
                            when (command.action) {
                                ...
                                SessionControlAction.PLAY_START_SOUND -> Unit
                            }
                        }
            ...
            SessionOperatingMode.DISPLAY_HOST -> {
                when (event) {
                    ...
                    is NearbyEvent.EndpointLost -> {
                        ...
                        displayGameModeAutoReductionMillisByEndpointId.remove(event.endpointId)
                        ...
                        if (!handledControllerIdentity) {
                            ...
                            when (command.action) {
                                SessionControlAction.RESET_TIMER -> {
                                    ...
                                    displayGameModeCurrentLivesByEndpointId[command.targetEndpointId] = configuredLives
                                ...
                                SessionControlAction.SET_GAME_MODE_AUTO_CONFIG -> {
                                    ...
                                    val everyRuns = (command.gameModeAutoEveryRuns ?: DEFAULT_GAME_MODE_AUTO_EVERY_RUNS)
                                        .coerceIn(1, 20)
                                    val reductionMillis =
                                        (command.gameModeAutoReductionMillis ?: DEFAULT_GAME_MODE_AUTO_REDUCTION_MILLIS)
                                            .coerceIn(50L, 500L)
                                    ...
                                    displayGameModeAutoReductionMillisByEndpointId[command.targetEndpointId] = reductionMillis
                                    displayGameModeAutoRunCountByEndpointId[command.targetEndpointId] = 0
                                }
                                SessionControlAction.PLAY_START_SOUND -> {
    ...
    private fun syncControllerSummaries() {
        ...
        val monitoringActiveForLocalCapture = raceState.monitoringActive &&
            mode != SessionOperatingMode.DISPLAY_HOST &&
            !isPassiveDisplayClient
        syncLocalCaptureLifecycle(
            raceState = raceState,
            monitoringActive = monitoringActiveForLocalCapture,
            shouldRunLocalCapture = shouldRunLocalCapture,
            isLocalMotionMonitoring = motionBefore.monitoring,
        )
        val hasDisplayLocalWaitingTimers = mode == SessionOperatingMode.DISPLAY_HOST &&
            displayWaitingEndpointIds.any { endpointId ->
                displayLatestLapByEndpointId[endpointId] == null &&
                    displayWaitTextEnabledByEndpointId[endpointId] != true &&
                    displayWaitingStartElapsedRealtimeNanosByEndpointId[endpointId] != null
            }
        syncTimerLifecycle(
            raceState = raceState,
            monitoringActive = monitoringActiveForLocalCapture,
            hasDisplayLocalWaitingTimers = hasDisplayLocalWaitingTimers,
        )
        relaySingleDeviceLapMessagesIfNeeded(
            mode = mode,
            raceState = raceState,
        )
        val projection = buildMonitoringProjection(
            raceState = raceState,
            localRole = localRole,
            isPassiveDisplayClient = isPassiveDisplayClient,
        )
        publishMonitoringUiState(projection)
    }

    private fun syncLocalCaptureLifecycle(
        raceState: com.paul.sprintsync.feature.race.domain.RaceSessionUiState,
        ...
        when (
            resolveLocalCaptureAction(
                ...
                shouldRunLocalCapture = shouldRunLocalCapture,
                isLocalMotionMonitoring = isLocalMotionMonitoring,
    ...
    private fun syncTimerLifecycle(
        raceState: com.paul.sprintsync.feature.race.domain.RaceSessionUiState,
        monitoringActive: Boolean,
        hasDisplayLocalWaitingTimers: Boolean,
    ) {
        if (
    ...
    private fun relaySingleDeviceLapMessagesIfNeeded(
    ...
    private fun buildMonitoringProjection(
        raceState: com.paul.sprintsync.feature.race.domain.RaceSessionUiState,
        localRole: SessionDeviceRole,
        isPassiveDisplayClient: Boolean,
    ): MainActivityMonitoringProjection {
        val motionState = motionDetectionController.uiState.value
        val nowSensorNanos = SystemClock.elapsedRealtimeNanos()
        val nowDisplayElapsedRealtimeNanos = SystemClock.elapsedRealtimeNanos()
        return buildMainActivityMonitoringProjection(
            input = MainActivityMonitoringProjectionInput(
                mode = raceState.operatingMode,
                networkRole = raceState.networkRole,
                stage = raceState.stage,
                monitoringActive = raceState.monitoringActive,
                timeline = raceState.timeline,
                latestCompletedTimeline = raceState.latestCompletedTimeline,
                ...
                connectedEndpoints = connectionsManager.connectedEndpoints(),
                currentNetworkRoleLabel = connectionsManager.currentRole().name.lowercase(),
                ...
                displayControllerEndpointIds = displayControllerEndpointIds,
                ...
                displayWaitingStartElapsedRealtimeNanosByEndpointId = displayWaitingStartElapsedRealtimeNanosByEndpointId,
                displayWaitTextEnabledByEndpointId = displayWaitTextEnabledByEndpointId,
                displayLimitMillisByEndpointId = displayLimitMillisByEndpointId,
                displayGameModeEnabledByEndpointId = displayGameModeEnabledByEndpointId,
                displayGameModeLimitMillisByEndpointId = displayGameModeLimitMillisByEndpointId,
                displayGameModeConfiguredLivesByEndpointId = displayGameModeConfiguredLivesByEndpointId,
                displayGameModeCurrentLivesByEndpointId = displayGameModeCurrentLivesByEndpointId,
                connectedWifiSsid = currentConnectedWifiSsid(),
                targetMonitoringWifiSsid = TARGET_MONITORING_WIFI_SSID,
                monitoringConfig = motionState.config,
                monitoringObservedFps = motionState.observedFps,
                monitoringTargetFpsUpper = motionState.targetFpsUpper,
                monitoringRawScore = motionState.rawScore,
                monitoringBaseline = motionState.baseline,
                monitoringEffectiveScore = motionState.effectiveScore,
                monitoringLastFrameSensorNanos = motionState.lastFrameSensorNanos,
                monitoringStreamFrameCount = motionState.streamFrameCount,
                monitoringProcessedFrameCount = motionState.processedFrameCount,
                monitoringIsActive = motionState.monitoring,
                triggerHistory = motionState.triggerHistory.map { trigger ->
                ...
                hostIpFallback = BuildConfig.TCP_HOST_IP,
                hostPort = BuildConfig.TCP_HOST_PORT,
            ),
        )
    }

    private fun publishMonitoringUiState(projection: MainActivityMonitoringProjection) {
        updateUiState {
            copy(
                stage = projection.stage,
                operatingMode = projection.operatingMode,
                networkRole = projection.networkRole,
                sessionSummary = projection.sessionSummary,
                monitoringSummary = projection.monitoringSummary,
                userMonitoringEnabled = projection.userMonitoringEnabled,
                clockSummary = projection.clockSummary,
                startedSensorNanos = projection.startedSensorNanos,
                stoppedSensorNanos = projection.stoppedSensorNanos,
                devices = projection.devices,
                canStartMonitoring = projection.canStartMonitoring,
                isHost = projection.isHost,
                localRole = projection.localRole,
                monitoringConnectionTypeLabel = projection.monitoringConnectionTypeLabel,
                hasConnectedPeers = projection.hasConnectedPeers,
                wifiWarningText = projection.wifiWarningText,
                runStatusLabel = projection.runStatusLabel,
                runMarksCount = projection.runMarksCount,
                elapsedDisplay = projection.elapsedDisplay,
                threshold = projection.threshold,
                roiCenterX = projection.roiCenterX,
                roiWidth = projection.roiWidth,
                roiCenterY = projection.roiCenterY,
                roiHeight = projection.roiHeight,
                cooldownMs = projection.cooldownMs,
                processEveryNFrames = projection.processEveryNFrames,
                observedFps = projection.observedFps,
                cameraFpsModeLabel = projection.cameraFpsModeLabel,
                targetFpsUpper = projection.targetFpsUpper,
                rawScore = projection.rawScore,
                baseline = projection.baseline,
                effectiveScore = projection.effectiveScore,
                frameSensorNanos = projection.frameSensorNanos,
                streamFrameCount = projection.streamFrameCount,
                processedFrameCount = projection.processedFrameCount,
                triggerHistory = projection.triggerHistory,
                splitHistory = projection.splitHistory,
                discoveredEndpoints = projection.discoveredEndpoints,
                connectedEndpoints = projection.connectedEndpoints,
                networkSummary = projection.networkSummary,
                displayLapRows = projection.displayLapRows,
                displayConnectedHostName = projection.displayConnectedHostName,
                displayConnectedHostEndpointId = projection.displayConnectedHostEndpointId,
                displayDiscoveryActive = projection.displayDiscoveryActive,
                controllerTargetEndpoints = projection.controllerTargetEndpoints,
            )
        }
    }
    ...
    private fun sendControllerCommandToDisplayHost(
        ...
        gameModeAutoReductionMillis: Long? = null,
    ) {
        val hostEndpoint = displayConnectedHostEndpointId
    ...
    private fun playStartSoundEffect() {
        runCatching {
            startSoundPlayer?.release()
            startSoundPlayer = MediaPlayer.create(this, R.raw.start)?.apply {
            ...
            appendEvent("start sound error: ${error.localizedMessage ?: "unknown"}")
        }
    }

    private fun applyGameModeLifePenaltyIfNeeded(endpointId: String, elapsedNanos: Long) {
        if (displayGameModeEnabledByEndpointId[endpointId] == false) {
            return
        }
    ...
    private fun applyGameModeAutoLimitProgressIfNeeded(endpointId: String) {
        ...
        if (displayGameModeAutoEnabledByEndpointId[endpointId] == false) {
            return
        }
        ...
        val reductionMillis =
            displayGameModeAutoReductionMillisByEndpointId[endpointId] ?: DEFAULT_GAME_MODE_AUTO_REDUCTION_MILLIS
    ...
    private fun clearDisplayHostLapState() {
        ...
        displayGameModeAutoReductionMillisByEndpointId.clear()
...
internal fun buildDisplayLapRowsForConnectedDevices(
    ...
    val orderedEndpointIds = connectedEndpointIds.toList().let { endpointIds ->
        val (pixelEndpoints, otherEndpoints) = endpointIds.partition { endpointId ->
            deviceNamesByEndpointId[endpointId]?.contains("pixel 7", ignoreCase = true) == true
        }
        pixelEndpoints + otherEndpoints
    }
    return orderedEndpointIds.map { endpointId ->
        ...
        val waitTextEnabled = waitTextEnabledByEndpointId[endpointId] == true
        ...
        val gameModeEnabled = gameModeEnabledByEndpointId[endpointId] != false
        val gameModeCountdownEnabled = gameModeEnabled
        val gameModeLimitMillis = gameModeLimitMillisByEndpointId[endpointId] ?: 5_000L
        val effectiveLimitMillis = if (gameModeCountdownEnabled) gameModeLimitMillis else null
        val remainingMillis = if (effectiveLimitMillis != null) {
            val elapsedMillis = elapsedNanos?.div(1_000_000L)
            if (elapsedMillis != null) {
                effectiveLimitMillis - elapsedMillis
            } else {
        ...
        val lapTimeLabel = when {
            remainingMillis != null -> formatSignedMillisDisplay(remainingMillis)
            elapsedNanos != null -> {
        ...
        val isOverLimit = remainingMillis != null && elapsedNanos != null && remainingMillis <= 0L
        val isUnderLimit = remainingMillis != null && endpointFinalNanos != null && remainingMillis > 0L
...
internal fun formatDisplayLimitLabel(limitMillis: Long): String = "${limitMillis} ms"

internal fun formatSignedMillisDisplay(totalMillis: Long): String =
    String.format(java.util.Locale.US, "%.3f", totalMillis / 1_000.0)
